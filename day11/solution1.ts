...
const getPosition = (seatMap: ILayout, row: number, col: number) => {
  if (!seatMap[row]) return undefined;
...
const getNeighbours = (
  ...
  return [
    getPosition(seatMap, row - 1, col - 1),
    getPosition(seatMap, row - 1, col),
    getPosition(seatMap, row - 1, col + 1),
    getPosition(seatMap, row, col - 1),
    getPosition(seatMap, row, col + 1),
    getPosition(seatMap, row + 1, col - 1),
    getPosition(seatMap, row + 1, col),
    getPosition(seatMap, row + 1, col + 1),
...
const checkRules = (
  ...
  if (
    pos.isOccupied &&
    getNeighbours(seatMap, row, col).filter((x) => x.isOccupied).length >= 4
  ) {
